...
These docs track the `0.3` prerelease source. The `next` channel installs the latest published candidate and can lag this branch; untagged `latest` stays on `0.2` until the stable cutover.

This prerelease supports Nuxt 4.4.7 through Nuxt 4.x, Vue 3.5.35 through Vue 3.x, and Node.js 22.18–22.x, 24.11–24.x, or 26+.
