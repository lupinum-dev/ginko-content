...
Run `nuxt prepare` before a CMS tool reads the application contract. The module
generates `.ginko/content-contract.json` from the same resolved settings used by
the content runtime. Do not edit or commit this file.

```ts [scripts/read-content-contract.ts]
import { readResolvedContentContract } from '@lupinum/ginko-content/cms-contract/node'

const { contract, sha256 } = await readResolvedContentContract({
  root: process.cwd()
})
```

Use `contract` to configure CMS fields, localization, routes, references, and
portable components. Store `sha256` with a deployment or import receipt. Reject
an operation when its expected hash does not match. Do not reconstruct the
contract from `content.config.ts` and Nuxt options in another package.
...
1. Read the resolved contract.
2. Implement the required `query()` method.
3. Declare the operators and pagination modes that the backend executes.
4. Return normalized provider documents.
5. Add optional operations only when the backend has an efficient implementation.
6. Run source conformance.

If a CMS producer is deployed separately from the Nuxt application, wrap every
result with `createCmsProviderWireEnvelope()` from `cms-contract`. The consumer
parsers require the matching `CMS_PROVIDER_WIRE_PROTOCOL`; package/component
skew then fails clearly before Ginko interprets backend data.
...
Use the cache-hint constructor when one operation has cache facts:

```ts [server/cache-hint.ts]
import { createContentDataSourceCacheHint } from '@lupinum/ginko-content/data-source'

export const cache = createContentDataSourceCacheHint({
...
Ginko validates one hint and owns all merge behavior. Do not merge tags, paths,
or ETags in the adapter.

## Test with the in-memory source

Use the package fixture for tests and development tools that do not need a real
database:

```ts [test/content-source.ts]
import {
  createDefaultProviderFixture,
  createFixtureContentDataSource
} from '@lupinum/ginko-content/testing/provider-fixture'

export const source = createFixtureContentDataSource(
  createDefaultProviderFixture()
)
```

This fixture is test infrastructure. It is not a production storage adapter.

## Translate the query at one boundary

| Backend | Adapter responsibility |
| --- | --- |
| Convex | Translate filters, ordering, and cursors to indexed Convex queries. |
| Postgres | Translate the bounded plan to parameterized SQL and an opaque cursor. |
| Workers or Durable Objects | Use Worker bindings inside the adapter context. Keep binding names and object IDs private. |
| REST or GraphQL CMS | Translate one bounded operation to the remote API. Do not expose its query language through Ginko types. |

The data-source result is identical for every backend. Database table names,
Convex function references, SQL, bindings, and remote query documents do not
...
### Find and remap stored media

Use Content's asset helpers when an application stores media IDs in Markdown
or MDC. The helpers read native images and the media source property declared
by each component policy. Text, links, and code examples are not media references.

```ts [app/remap-media.ts]
import type { PortableComponentPolicy } from '@lupinum/ginko-content/cms-contract'
import {
  collectStoredMdcAssetReferences,
  rewriteStoredMdcAssetReferencesForStorage
} from '@lupinum/ginko-content/portability'

export async function remapMedia(
  source: string,
  policy: PortableComponentPolicy,
  replacements: ReadonlyMap<string, string>
) {
  const referencedIds = await collectStoredMdcAssetReferences(source, policy)
  const rewritten = await rewriteStoredMdcAssetReferencesForStorage(
    source,
    policy,
    id => replacements.get(id) ?? id
  )
  return { referencedIds, source: rewritten }
}
```

Collection returns one ID per occurrence in document order. Both helpers reject
documents outside the supplied policy. Handle that error before deleting any
assets; a rejected document does not mean that it has no references.

The rewrite callback can be asynchronous and must return a valid stored ID.
If no ID changes, the source remains byte-for-byte identical. Otherwise, Content
serializes and validates the changed document. The existing
`rewriteStoredMdcAssetReferences` helper instead converts stored IDs to portable
asset paths or public URLs for export.
...
verified context. Its found, missing, list, count, two-page cursor, and optional
operation probes execute through the real binder. Offset adapters must provide
a count probe. Every implemented optional operation must provide a
discriminating probe. Keep `runContentDataSourceContract` for a focused
single-query probe. Core's binder contract owns adversarial response, cache,
cancellation, deadline, JSON-purity, and route-limit tests. Adapters must not
duplicate those invariants. A passing Level 1 result proves the public protocol
and bounds only.
...
a stale or scope-invalid query cursor. Throw
`createContentDataSourceError('QUERY_UNSUPPORTED')` when the bounded plan cannot
be executed. Throw `createContentDataSourceError('BACKEND_FAILURE')` for a
sanitized backend failure. Do not include backend messages or details. The
public boundary maps query errors to 400 and backend failures to 502. Binder
deadlines map to 504. Invalid provider responses map to 502.
