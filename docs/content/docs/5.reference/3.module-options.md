  ...
  content: {
    search: {
      ...
      minisearch: {
        ...
        storeFields: ['path', 'title', 'excerpt', 'anchor', 'locale', 'collection', 'tags'],
...
| `minisearch.storeFields` | `string[]` | `['path', 'title', 'excerpt', 'anchor', 'locale', 'collection']` |
...
The required `path`, `title`, and `excerpt` store fields are retained even when `storeFields` is customized. Add a frontmatter field to `extraFields` before indexing or storing it. [Choose a search backend](/docs/guides/search-engines) explains the deployment trade-offs.
