...
| `fields.image()` | `{ aspectRatio?, accept? }` | Managed image reference string |
| `fields.asset()` | `{ accept? }` | Managed image reference string |
| `fields.file()` | `{ accept? }` | Managed image reference string; general files are not supported yet |
...
Managed assets currently support PNG, JPEG, GIF, and WebP up to 25 MiB. When
`accept` is omitted, all four formats are allowed. Supplying any other MIME type
fails contract generation instead of silently weakening the field policy.
General files such as PDFs are not managed or portable yet; use a canonical
external HTTPS reference when the content model needs one.
...
| Option | Fields |
|---|---|
| `site` | Required `title`, `description`, and `whenToUse`; optional `whenNotToUse` and `contentSignals.search`, `aiInput`, `aiTrain` |
...
Configure the canonical deployment origin as Nuxt `site.url`. Agent output does
not accept a separate URL because two origin settings can produce broken
canonical links and indexes. Production agent routes require this value even
