...
`where` accepts field comparisons and the logical operators `$and`, `$or`, and
`$not`. Provider-backed queries may use only comparison operators advertised by
that provider. Logical nodes are required structural parts of the v3 provider
wire rather than separate capabilities.
