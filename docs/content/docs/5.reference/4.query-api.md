...
```ts
// Client or SSR Vue
import { count, many, one, paginate } from '@lupinum/ginko-content/client'

// Nitro
import { count, many, one, paginate } from '@lupinum/ginko-content/server'
```
...
Returns the exact number of matching documents without transferring document
bodies. It accepts `where`, `locale`, and `fallback`:

```ts
const publishedPostCount = await count(blog, {
  where: { draft: { $ne: true } }
})
```

Use `count()` for totals. Do not fetch `many()` and read the array length:
`many()` has a bounded result size and may intentionally return only the first
100 matches. A missing collection result maps to `0`; malformed queries and
...
Population runs behind Ginko's server query boundary. A browser call still
makes one request, even when the server performs several bounded provider reads
to resolve references. Provider implementations continue to receive the stable
v4 query contract; `populate` is transport metadata owned by Ginko.
