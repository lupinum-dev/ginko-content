...
missing document, failed query, or stale document
...
result, Ginko calls the optional `fallback` function only when the requested
locale is in the global content locale configuration (`runtime.locales`).
Otherwise, it returns `undefined` without calling the function. Ginko returns
the callback's value unchanged. Content pages use their document's alternates,
including collection-specific locales. A content page with no translation never
calls application fallback. Callback errors propagate.
