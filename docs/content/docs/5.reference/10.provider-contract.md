  ...
  async query(event, query) {
    switch (query.plan.mode) {
      ...
      case 'all': {
        ...
        const paging = query.plan.paging
        return {
          ...(paging?.mode === 'offset' ? { mode: 'offset' as const } : {}),
          result: page.items,
          skip: paging?.mode === 'offset' ? paging.skip : query.plan.skip,
          limit: paging?.mode === 'offset'
            ? paging.limit
...
For multi-locale `routeVariants`, `canonicalKey` is required and the entry for the row's locale must equal `contentPath`. Return only variants visible under the current request policy. When `plan.variantSelector` exists, return its first matching candidate in order so core can label a proven fallback route.
...
The wrapper cannot collide with document fields named `data` or `cache`. Providers describe dependencies and freshness; the application-owned `ContentCacheAdapter` applies headers and invalidates host caches.
