...
const source = await readFile(resolve(root, 'docs/content/docs/1.get-started/1.quickstart.md'), 'utf8')
const [start, finish] = bounds(source)
const block = source.slice(start, finish)
for (const file of ['README.md', 'packages/content/README.md']) {
  const path = resolve(root, file)
  const current = await readFile(path, 'utf8')
  const [from, to] = bounds(current)
  if (process.argv.includes('--write')) await writeFile(path, current.slice(0, from) + block + current.slice(to))
  else if (current.slice(from, to) !== block) throw new Error(`${file} onboarding is stale. Run pnpm docs:onboarding.`)
}
