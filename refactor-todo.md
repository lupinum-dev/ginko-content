...
- **T3.1a — Buckets A+B.** In `src/types/content.ts`, define `ContentDocument` with the Bucket-A top-level fields and the Bucket-B nested optional `file` object; rename `_canonicalKey`→`canonicalKey` (value unchanged; JSDoc: opaque identity key, never a URL) through the graph/manifest layer too (`core/content/graph.ts`, `parsers/path-meta.ts`). Every stored-document `_path` → `path`. The shaped "canonical slot" write-sites (`features/localization/results.ts:160-167,192-197`, `features/navigation/canonical.ts:220-227`) are **deleted per the 2026-07-08 decision**: first enumerate readers (`git grep -n "\._path" packages/content/src/runtime/app packages/content/src/public`), migrate identity-fallback readers to `canonicalKey`, filter/debug readers to `path`, then remove the writes. Then chase the compiler. Expected blast radius: ~35-50 src files + tests + `src/testing/provider-fixture.ts` + `examples/advanced/cms-cache-contract/`.
...
**Reserved-key guard (new, small):** after the rename, system-computed identity keys (`id`, `collection`, `locale`, `path`, `canonicalKey`, `type`, `file`) are reserved. During parse, if user frontmatter defines one, the system value wins and a **dev/build warning** names the file and key. One test.
