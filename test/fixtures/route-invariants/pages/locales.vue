...
const enGettingStarted = await one(docs, { by: { path: '/getting-started' }, locale: 'en' })
const deGettingStarted = await one(docs, { by: { path: '/erste-schritte' }, locale: 'de' })
...
<template>
  <pre data-testid="locales">{{ JSON.stringify({
    ...
    fallbackResolvedLocale: deFallback?.resolution.resolved.locale,
