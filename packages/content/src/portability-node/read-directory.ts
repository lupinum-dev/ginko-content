...
export async function rebuildPortableDirectoryManifest(root: string): Promise<PortableManifestV1> {
  const result = await inspectPortableDirectoryBounded(root, false)
...
export async function verifyPortableDirectoryBounded(
  ...
  return await inspectPortableDirectoryBounded(root, true)
}

async function inspectPortableDirectoryBounded(
  root: string,
